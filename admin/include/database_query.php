<?php
    ...
    * * * * * * * * * * * * * * * * * * * * * */
    ...
    {
        global $conn, $committee_id, $committee_type_id, $committee_type_name, $committee_name, $description, $username, $user_id, $published, $file, $errors;
    ...
    function updateCommittee($request_values)
    {
        global  $conn, $committee_type_id, $committee_type_name, $committee_name, $description, $username, $user_id, $is_published, $file, $errors;
        // get id of the notice to be updated
        $id = $request_values['committee_id'];
        ...
        $mainQueryBody = "name = '$committee_name', type_id = '$committee_type_id', type_name = '$committee_type_name', description = '$description',is_published = $is_published, updated_time = now()";
        ...
        if (!empty($_FILES['file']['name'])) {
            ...
            $mainQueryBody = $mainQueryBody . ", file = '$new_image_name'";
            // image file directory
            $target = "../upload/committee/" . $new_image_name;
            if (!move_uploaded_file($_FILES['file']['tmp_name'], $target)) {
                // array_push($errors, "Failed to upload image. Please check file settings for your server");
        ...
        // submit if there are no errors in the form
        if (count($errors) == 0) {
            $query = "UPDATE committee SET $mainQueryBody where id='$id'";
            if (mysqli_query($conn, $query)) { // if  created successfully
                // $inserted_post_id = mysqli_insert_id($conn);
                // set edit state to false
                $isEditingCommittee = false;

                $_SESSION['message'] = "Committee Updated successfully";
                echo "<script language='javascript'>
